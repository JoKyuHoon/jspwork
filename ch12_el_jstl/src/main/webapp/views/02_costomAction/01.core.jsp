...
<body>
	...
	* 변수 선언과 동시에 초기화
	...
	- 해당 변수를 어떤 scope에 담아 둘것인지 지정 가능함(생략시 pageScope)
	  ...
	  ex) pageScope.setAttibute("변수명","값")		
	- 변수 타입을 변도로 지정하지 않음
	- 초기값은 반드시 지정해야됨
	...
	<c:set var="num1" value="10"/> <!--pageContext.setAttibute("num1","10")  -->
	...
	* 변수 삭제(c:remove var="제거하고자하는 변수명" [scope=""])
	...
	- scope를 지정하지 않으면 모든 scope에서 해당 변수를 모두 제거
	</pre>
	
	삭제전 result : ${result}<br><br>
	
	1) 특정 scope(request)를 지정하여 삭제<br>
	<c:remove var="result" scope="request"/>
	request의 result 삭제 후 : ${result }<br><br>
	
	2) 모든 scope에서 삭제<br>
	<c:remove var="result" />
	모두 삭제한 후 result : ${result}<br><br>
	...
	* 변수 출력 (c:out value="출력하고자하는 값" [default="기본값"] [escapeXml="true/false"])
	...
	a : <c:out value="${a}" default="없음"/><br><br>
	
	<c:set var="outTest" value="<b>출력테스트</b>"/>
	<c:out value="${outTest}"/><br> <!--escapeXml을 생략시 기본값 true => 문자열로 인식  -->
	<c:out value="${outTest}" escapeXml="false"/> <!--escapeXml을 넣어야 <b>태그 적용  -->
	 ...
	 * 향상된 for문
	 - (c:forEach var="변수명" items="순차적으로 접근할 객체(배열|컬렉션)"[varStatus="현재 접근한 요소의 상태값을 보관할 변수명"])
	 </pre>
	...
	<%
		...
		list.add(new Person("고길동", 21, "남자"));
		list.add(new Person("남궁길동", 23, "여자"));
	%>
	...
	* 구분자를 통해서 분리된 각각의 문자열에 순차적으로 접근하여 반복 수행
	...
	<pre>
	- url 경로를 생성하고 쿼리 스트링을 정의해 둘 수 있는 태그
